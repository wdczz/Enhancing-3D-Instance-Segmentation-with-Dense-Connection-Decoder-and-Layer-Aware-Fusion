...
import time
from dcd.utils import cuda_cast, rle_encode
...
class DCD(nn.Module):
    ...
    def __init__(
        ...
        self.backbone = SpUnet( input_channel = input_channel,
                        blocks = blocks,
        ...
        self.pool = pool
        if pool == "geo":
            self.pooling = GeoAwarePooling(media)
    ...
    def loss(self, scan_ids, voxel_coords, p2v_map, v2p_map, spatial_shape, feats, insts, superpoints, coords_float, batch_offsets):
        ...
        sp_feats = self.extract_feat(input, coords_float, superpoints, p2v_map)
    ...
    def predict_by_feat(self, scan_ids, out, superpoints, insts):
        ...
        mask_pred = (mask_pred_sigmoid > 0.4).int()
    ...
    def extract_feat(self, x, coords_float, superpoints, v2p_map):
        # backbone
        x = self.backbone(x)
        ...
        if self.pool == 'mean':
            ...
            x = self.pooling(x, superpoints, coords_float)
        return x
...
class GeoAwarePooling(nn.Module):
    ...
    def forward(self, x, sp_idx, all_xyz):
        all_xyz, _ = self.scatter_norm(all_xyz, sp_idx)
