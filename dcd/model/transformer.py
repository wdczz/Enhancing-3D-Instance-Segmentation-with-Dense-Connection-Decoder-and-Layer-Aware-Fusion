...
class TransformerDecoder(nn.Module):
    ...
    def __init__(self, decoder_layer, num_layers, norm=None, return_intermediate=False, nhead=8, d_model=256, temperature=10000, pos_type="fourier", attn_mask_thresh=0.5):
        ...
        # RWKV Init
        # fancy time_decay
        self.time_decay = nn.ParameterList([
            nn.Parameter(torch.ones(400, 1) * (1e-4 + 1e-4 * (math.exp(i) - 1)))
            for i in range(num_layers-1)
        ])
        self.time_first = nn.ParameterList([
            nn.Parameter(torch.ones(400, 1) * (1e-4 + 1e-4 * (math.exp(i) - 1)))
            for i in range(num_layers-1)
        ])

        # fancy time_mix
        self.time_mix_k = nn.ParameterList([nn.Parameter(torch.rand(400, 1)) for _ in range(num_layers-1)])
        self.time_mix_v = nn.ParameterList([nn.Parameter(torch.rand(400, 1)) for _ in range(num_layers-1)])
        self.time_mix_r = nn.ParameterList([nn.Parameter(torch.rand(400, 1)) for _ in range(num_layers-1)])

        # function
        self.tm_key = nn.Sequential(
        ...
        self.tm_receptance = nn.Sequential(
            nn.Linear(d_model, 2 * d_model, bias=False),
            nn.GELU(),
            nn.Linear(2 * d_model, d_model, bias=False), 
            nn.LayerNorm(d_model)
    ...
    def timing_mix(self, layer_id, x, xx, aa, bb, p = None):
        """
        Layer_id : int
        x : queries.shape [B, N, D]
        xx : None / queries.shape [B, N, D]
        aa : queries.shape [B, N, D]
        bb : queries.shape [B, N, D]
        p : None / queries.shape [B, N, D]
        """
        B = x.shape[1]
        if layer_id == 0:
            xk = x
            ...
            k = self.tm_key(xk)
            v = self.tm_value(xv)
            r = self.tm_receptance(xr)
            sr = torch.sigmoid(r)
            wkv = (torch.exp(k) * v)/ (torch.exp(k))
            aa = v
            bb = torch.ones_like(k)
            p = k
        else:
            time_k = self.time_mix_k[layer_id-1].unsqueeze(1).repeat(1, B, 1) # [N, 1, 1] -> [N, B, 1]
            time_v = self.time_mix_v[layer_id-1].unsqueeze(1).repeat(1, B, 1) # [N, 1, 1] -> [N, B, 1]
            time_r = self.time_mix_r[layer_id-1].unsqueeze(1).repeat(1, B, 1) # [N, 1, 1] -> [N, B, 1]
            xk = x * time_k + xx * (1 - time_k) if xx is not None else x
            xv = x * time_v + xx * (1 - time_v) if xx is not None else x
            xr = x * time_r + xx * (1 - time_r) if xx is not None else x
            k = self.tm_key(xk)
            v = self.tm_value(xv)
            r = self.tm_receptance(xr)
            sr = torch.sigmoid(r)
            u = self.time_first[layer_id-1].unsqueeze(1).repeat(1, B, 1) # [N, 1, 1] -> [N, B, 1]
            w = self.time_decay[layer_id-1].unsqueeze(1).repeat(1, B, 1) # [N, 1, 1] -> [N, B, 1]
            ww = u + k
            qq = torch.maximum(p, ww)
            e1 = torch.exp(p - qq)
            e2 = torch.exp(qq - ww)
        ...
        # print(x.shape, aa.shape, bb.shape, p.shape)
        return self.tm_output(sr * wkv), aa, bb, p
    ...
    def forward(self, tgt, memory, input_ranges, coords_float, mask_feats_batched, lengths,
        ...
        aa = torch.zeros_like(output)
        bb = torch.zeros_like(output)
        tm_token_shift = None
        pp = None

        for layer_id, layer in enumerate(self.layers):

            o, aa, bb, pp = self.timing_mix(layer_id, output, tm_token_shift, aa, bb, pp)
            output = self.ln1(o) + output
            tm_token_shift = output
